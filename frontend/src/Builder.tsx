...
function Builder({ language = 'EN' }: BuilderProps) {
  ...
  const downloadFullProject = async () => {
    ...
    testRunner += "        console.log('  💼 Checking for existing Polygon Wallet...');\n";
    testRunner += "        // First try to GET an existing wallet\n";
    testRunner += "        let existingWalletId = '';\n";
    testRunner += "        try {\n";
    testRunner += "          const getWalletRes = await axios.get('https://synthesis.trade/api/v1/wallet', {\n";
    testRunner += "            headers: { 'X-API-KEY': accountApiKey }\n";
    testRunner += "          });\n";
    testRunner += "          if (getWalletRes.data?.success && getWalletRes.data.response?.length > 0) {\n";
    testRunner += "            existingWalletId = getWalletRes.data.response[0].wallet_id;\n";
    testRunner += "            const polyAddr = getWalletRes.data.response[0].chains?.POL?.address || 'unknown';\n";
    testRunner += "            console.log('  ✅ Found Existing Wallet: ' + existingWalletId);\n";
    testRunner += "            console.log('  📍 Polygon Address: ' + polyAddr);\n";
    testRunner += "            walletId = existingWalletId;\n";
    testRunner += "          }\n";
    testRunner += "        } catch(err) {}\n\n";
    testRunner += "        // If no wallet exists, create a new one\n";
    testRunner += "        if (!walletId) {\n";
    testRunner += "          console.log('  💼 Creating new Polygon Wallet...');\n";
